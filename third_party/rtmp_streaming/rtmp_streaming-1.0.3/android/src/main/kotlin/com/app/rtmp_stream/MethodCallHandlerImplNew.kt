...
class MethodCallHandlerImplNew(
    ...
    private val viewType: String = "hybrid-view-type"
    ...
    init {
        ...
        platformViewRegistry.registerViewFactory(
            viewType,
            nativeViewFactory as NativeViewFactory
        )
    }

    private fun ensureWorkerHandler(): Handler {
        ...
        val newThread = HandlerThread("WorkerThread")
        newThread.start()
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "switchCamera" -> {
                ...
                val targetCameraName = call.argument<String>("cameraName")

                if (!targetCameraName.isNullOrEmpty()) {
                    nativeViewFactory?.cameraName = targetCameraName
                }

                getCameraView()?.switchCamera(targetCameraName, result)
            }
            ...
            "dispose" -> {
                ...
                // handlerThread/handler НЕ трогаем
                // nativeViewFactory НЕ сбрасываем
                // dartMessenger НЕ сбрасываем
            }
    ...
    private fun instantiateCamera(call: MethodCall, result: MethodChannel.Result) {
        val currentHandler = ensureWorkerHandler()
        ...
        currentHandler.postDelayed({
            if (activity.isFinishing) {
                activity.runOnUiThread {
                    result.error("activity_destroyed", "Activity was destroyed during initialization", null)
                }
                return@postDelayed
            }
            ...
            try {
                ...
                val size = previewSize["size"] as? Size ?: run {
                    activity.runOnUiThread {
                        result.error("CameraError", "Failed to compute preview size", null)
                    }
                    return@postDelayed
                }
